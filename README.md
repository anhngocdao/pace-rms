...
| Seasonal ladder, the incumbent | 79.1% | 180.84 | 143.04 | 118.52 | — |
...
tests/             29 checks over the parts where a quiet mistake is expensive
```
...
python3 run.py test               37 checks, about 8 seconds
```
...
## Two things I measured and did not like
...
The limit is the model, not the machine. This prices one room type; a real
hotel has five to fifteen with substitution between them, which is a network
problem. [METHOD.md](METHOD.md) section 13 is specific about where it stops.
